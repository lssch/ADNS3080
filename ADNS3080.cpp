...
ADNS3080::ADNS3080(SPI_HandleTypeDef*hspi_, Parameter::Vfs *parameter_)
  : hspi(hspi_),
...
uint8_t ADNS3080::init() {
  ...
  writeRegister(ADNS3080_CONFIGURATION_BITS, 0b00000000 | parameter->led_shutter << 6 | parameter->led_shutter << 4);
  motionClear();
  return SUCCESS;
}
...
void ADNS3080::motionBurst(Sensor::Vfs* vfs) {
  ...
  HAL_SPI_Transmit(hspi, tx_data, 1, ADNS3080_TIMEOUT);
  delay_us(ADNS3080_T_SRAD_MOT);

  // Get motion byte to see if motion occurred since the last call
  HAL_SPI_Receive(hspi, rx_data, 7, ADNS3080_TIMEOUT);
  ...
  if ((rx_data[0] & 0b10000000) >> 7){
    if (parameter->high_resolution)
      vfs->motion = {static_cast<float>(-static_cast<int8_t>(rx_data[2]))*2*parameter->height/(ADNS3080_PIXELS*1600)
                     *std::tan(static_cast<float>(parameter->measured_target_length)/static_cast<float>(parameter->height)),
                     static_cast<float>(static_cast<int8_t>(rx_data[1]))*2*parameter->height/(ADNS3080_PIXELS*1600)
                     *std::tan(static_cast<float>(parameter->measured_target_length)/static_cast<float>(parameter->height))};
    else
      vfs->motion = {static_cast<float>(-static_cast<int8_t>(rx_data[2]))*2*parameter->height/(ADNS3080_PIXELS*400)
                     *std::tan(static_cast<float>(parameter->measured_target_length)/static_cast<float>(parameter->height)),
                     static_cast<float>(static_cast<int8_t>(rx_data[1]))*2*parameter->height/(ADNS3080_PIXELS*400)
                     *std::tan(static_cast<float>(parameter->measured_target_length)/static_cast<float>(parameter->height))};
  } else
    vfs->motion = {0,0};
...
void ADNS3080::displacement(Sensor::Vfs* vfs) {
  ...
  // Get motion byte to see if motion occurred since the last call
  tx_data[0] = 0x00;
  HAL_SPI_Receive(hspi, rx_data, 3, ADNS3080_TIMEOUT);
  ...
  vfs->motion = {static_cast<float>(motion.x)*2*parameter->height/(ADNS3080_PIXELS*1600)*0.22595393133353245f,
                 static_cast<float>(motion.y)*2*parameter->height/(ADNS3080_PIXELS*1600)*0.22595393133353245f};
}
...
void ADNS3080::frameCapture() {
  ...
  while ((rx_data[0] & 0B01000000) == 0) {
    HAL_SPI_Receive(hspi, rx_data, 1, ADNS3080_TIMEOUT);
...
void ADNS3080::writeRegister(uint8_t reg, uint8_t data) {
  ...
  HAL_GPIO_WritePin(VFS_SPI_CS_GPIO_Port, VFS_SPI_CS_Pin, GPIO_PIN_RESET);
  if (HAL_SPI_Transmit(hspi, tx_data, 2, ADNS3080_TIMEOUT) != HAL_OK)
...
uint8_t ADNS3080::readRegister(uint8_t reg) {
  ...
  HAL_GPIO_WritePin(VFS_SPI_CS_GPIO_Port, VFS_SPI_CS_Pin, GPIO_PIN_RESET);
  if (HAL_SPI_Transmit(hspi, tx_data, 1, ADNS3080_TIMEOUT) != HAL_OK)
    std::cout << "Error while trying to write a register to VFS." << std::endl;
  delay_us(ADNS3080_T_SRAD_MOT);
  if (HAL_SPI_Receive(hspi, rx_data, 1, ADNS3080_TIMEOUT) != HAL_OK)
